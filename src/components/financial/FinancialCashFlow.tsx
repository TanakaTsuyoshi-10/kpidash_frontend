/**
 * キャッシュフローテーブル（財務分析用）
 * 営業CF、投資CF、財務CF、フリーCFを3年分+前年差で表示
 */
'use client'
...
export function FinancialCashFlow({ cashFlow, loading }: Props) {
  if (loading) {
    return (
      <Card>
        <CardHeader>
    ...
    <Card>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            {rows.map((row) => {
              const diff = getDiff(row.current, row.prev)
              return (
                <TableRow
                  key={row.label}
                ...
                >
                  ...
                  <TableCell className={cn('text-right', getValueColor(diff))}>
                    {formatDiff(diff)}
                  </TableCell>
                </TableRow>
              )
