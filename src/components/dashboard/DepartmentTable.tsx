...
export function DepartmentTable({ departments, loading }: Props) {
  ...
  const totals = departments.reduce(
    (acc, dept) => ({
      ...
      profit: (acc.profit || 0) + (dept.profit || 0),
    }),
    { sales: 0, profit: 0 }
  )
  ...
  const totalYoyRate = departments.length > 0
    ? departments.reduce((sum, d) => sum + (d.sales_yoy_rate || 0) * (d.sales || 0), 0) / totals.sales
    : null
  const totalAchievementRate = departments.length > 0
    ? departments.reduce((sum, d) => sum + (d.achievement_rate || 0) * (d.sales || 0), 0) / totals.sales
    : null
  const totalBudgetRate = departments.length > 0
    ? departments.reduce((sum, d) => sum + (d.budget_rate || 0) * (d.sales || 0), 0) / totals.sales
    : null
