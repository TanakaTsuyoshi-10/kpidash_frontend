    ...
    <div className="space-y-1">
      {stageNos.map((stageNo, stageIdx) => {
        ...
        return (
          <div key={stageNo}>
            {stageIdx > 0 && (
              <div className="flex justify-center text-gray-300 -my-0.5">
                <ArrowDownIcon className="h-3 w-3" />
              </div>
            )}
            <div
              className={`rounded-md border px-2.5 py-1 space-y-0.5 ${
                isCurrentStage ? 'border-amber-300 bg-amber-50' : 'border-gray-200'
              }`}
            >
              <div className="flex items-center gap-2">
                <span className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-100 text-gray-600 text-[11px] font-semibold flex items-center justify-center">
                  {stageNo}
                </span>
                {group.length > 1 && (
              ...
              {group.map((step) => (
                <div key={step.id} className="flex items-center gap-2 pl-1 py-0.5">
